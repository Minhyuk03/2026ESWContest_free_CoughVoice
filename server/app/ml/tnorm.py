...
`tools/build_tnorm_cohort.py`가 8/26 라벨 세션 아카이브(hwang·choi 88건, 전부 엣지
클립)로 만든다. **hwang·choi는 등록 화자 s01·s02·s04 중 누구도 아니다**(2026-08-28
사용자 확인) — 조건을 만족한다. 통제 녹음(collect_cough.py)은 채널이 달라 쓰지 않는다.

나중에 코호트에 든 사람을 화자로 등록하게 되면 `--exclude-speaker`로 빼고 다시 만들 것.
실제로 그 조건을 만들어 재현해봤다 — hwang을 등록한 채 코호트에 남겨두니 hwang의 기침이
다른 등록자로 판정됐고, 빼고 다시 재니 정확해졌다.
"""
