...
**한 창에 한 사람만 있다고 가정한다.** 발작은 한 사람의 기침 발작을 뜻하므로 가정집에서는
대체로 맞지만, **두 사람이 60초 안에 번갈아 기침하면 한 묶음으로 합쳐져 둘 다 한쪽 이름을
받는다.** 앞서 올바르게 붙은 이름까지 소급해서 덮어쓴다. 시연처럼 두 사람이 연달아 기침하는
상황에서는 사이를 60초 이상 띄워야 한다. 근본 해결은 창 안에서 화자별로 먼저 나누는 것인데
(클러스터링), 그러려면 이벤트별 임베딩을 저장해야 해서 지금 구조로는 안 된다.
