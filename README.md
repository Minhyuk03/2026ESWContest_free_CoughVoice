...
| 유효 채널 | LEFT만 사용 (RIGHT에도 같은 소리가 실린다 — 2026-08-28 재측정) |
...
- **기침 게이트 (PANNs CNN14, AudioSet 사전학습)**: 엣지의 에너지 검출을 통과한 박수·문소리·말소리를 서버에서 걸러낸다. 임계치는 연속 녹음 운용 곡선으로 정한다 — 3초 통제 샘플로 정하면 실환경에서 무너진다.
  **오탐은 확인됐다**: TV 켠 거실 **69.2분** 연속 녹음에서 게이트 통과 오탐 **0회**(95% 상한 2.6회/시간). 통과한 9건은 전부 진짜 기침이었다. 엣지 단계 오탐은 23.4회/시간으로 8/28 별도 측정(27회/시간)과 일치.
  **검출률은 무너졌다**: 같은 녹음을 사람이 귀로 라벨(기침 23 / 비기침 27)하니 임계치 0.05의 실환경 검출률이 **39.1%**였다. 종전 기록 "실환경 재현율 3/3"은 표본 3개가 만든 착시다. 라벨된 기침의 게이트 점수 중앙값은 **0.038** — 통제 녹음 기침(0.83~0.96)의 20분의 1로, 실환경 기침을 PANNs가 잘 못 알아본다.
  **임계치를 0.05 → 0.01로 내렸다**(2026-08-28). 종전 곡선은 엣지 하이패스 도입 *이전*에 그린 것이라 낡았다 — 그때는 엣지가 774회/시간을 올렸지만 지금은 23회/시간이라 게이트가 보는 후보가 34배 줄었다. 실측 곡선: `0.005 → 100%/3.5회` · **`0.010 → 82.6%/0.0회`** · `0.020 → 65.2%/0.0회` · `0.050 → 39.1%/0.0회`. 0.05는 검출률 43%p를 공짜로 버리고 있었다. 표본이 환경 하나·하루·기침 23개뿐이라 **0.01은 잠정값**이다.
  재현: `python3 tools/eval_ambient.py <녹음.wav> --threshold 0.05`
...
- **알림 엔진**: 관찰 규칙(횟수/야간) + 임상 근거 규칙(개인 기준선 대비 변화·지속일수·긴급 증상). "미등록 감지" 규칙은 근거가 없어 2026-08-28에 삭제했다 — 외부인 40건 중 35건(87.5%)이 임계치를 넘어 등록자 이름을 달았다. 문구는 `core/guidance.py`에서만 생성하고 모든 응답에 의료 면책을 동봉해 진단 표현이 새지 않게 차단.
...
- **실사용 조건에서는 동전 던지기다.** 2026-08-26 라벨 세션: 엣지가 평소대로 도는 상태에서 화자 2명이 번갈아 2블록씩 기침한 클립 80건 기준 **EER 46.3%**(동일인 800 / 타인 1600, `tools/eval_label_session.py`). 통제 녹음 36.4% → 실사용 46.3%.
- **"세션 인코딩"의 직접 증거.** 화자 내 유사도가 블록 *내부*는 +0.23~0.25인데 블록 *간*에는 +0.18~0.24로 떨어지고, 이는 타인 간(+0.18)과 같은 수준이다. 한 화자는 블록 간 +0.176으로 타인 간보다도 낮았다. 블록 내부 쌍으로만 쟀다면 "동작한다"고 오판했을 것이므로, 동일인 쌍은 반드시 블록 간으로 만든다.
- 논문 수준(EER 13.4%)은 백본 파인튜닝이 필요하고 CPU 전용 환경에선 비현실적.

**결론**: 화자 식별은 제품 기능으로 주장하지 않는다. 보고서에는 정량적 한계(통제 36.4% · 실사용 46.3%)와 원인(세션/블록 인코딩)을 재현 가능한 negative result로 기술한다.
